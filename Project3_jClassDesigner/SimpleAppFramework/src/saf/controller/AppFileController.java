...
public class AppFileController {
    ...
    //todo: add following methods to abstract classes (or somewhere), then implement
    
    public void handleSaveAsRequest() { 
        markFileAsNotSaved();
        handleSaveRequest();
	//app.getFileComponent().saveDataAs();
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }

    public void handlePhotoRequest() {
        app.getWorkspaceComponent().processPhotoTool();
    }
	...
	PropertiesManager props = PropertiesManager.getPropertiesManager();
		...
		JFileChooser fc = new JFileChooser();
                fc.setCurrentDirectory(new File(PATH_WORK));
                fc.setDialogTitle("Select a Directory for Exporting Code");
                fc.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
                fc.setAcceptAllFileFilterUsed(false);
                
                if(fc.showOpenDialog(null) == JFileChooser.APPROVE_OPTION){
                    if(fc.getSelectedFile() != null){
                        exportWork(fc.getSelectedFile());
                    }
	...
	// TELL THE USER THAT CODE HAS BEEN EXPORTED
	AppMessageDialogSingleton dialog = AppMessageDialogSingleton.getSingleton();
	PropertiesManager props = PropertiesManager.getPropertiesManager();
        dialog.show("Export Successful", "Your code was successfully exported.");
		    
	// AND REFRESH THE GUI, WHICH WILL ENABLE AND DISABLE
	// THE APPROPRIATE CONTROLS
	app.getGUI().updateToolbarControls(saved); //todo: edit boolean, update
        // (con't) the disabled status of only the exportCode button
    }

    public void handleSelectRequest() {
        app.getWorkspaceComponent().processSelectTool();
    }

    public void handleResizeRequest() {
        app.getWorkspaceComponent().processResizeTool();
    }

    public void handleAddClassRequest() {
        app.getWorkspaceComponent().processAddClass();
    }

    public void handleAddInterfaceRequest() {
        app.getWorkspaceComponent().processAddInterface();
    }

    public void handleRemoveRequest() {
        app.getWorkspaceComponent().processRemove();
    }

    public void handleUndoRequest() {
        app.getWorkspaceComponent().processUndo();
    }

    public void handleRedoRequest() {
        app.getWorkspaceComponent().processRedo();
    }

    public void handleZoomInRequest() {
        app.getWorkspaceComponent().processZoomIn();
    }

    public void handleZoomOutRequest() {
        app.getWorkspaceComponent().processZoomOut();
    }

    public void handleGridRequest(boolean isSelected) {
        app.getWorkspaceComponent().processGridToggle(isSelected);
    }

    public void handleSnapRequest(boolean isSelected) {
        app.getWorkspaceComponent().processSnapToggle(isSelected);
    }
}
