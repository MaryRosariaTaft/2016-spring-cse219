...
public class FileManager implements AppFileComponent {
    ...
    public void exportData(AppDataComponent data, String filePath) throws IOException {
        ...
        String fileContents = "<!DOCTYPE html> \n";
        
        // ACCESS THE DATA MANAGER
        DataManager dataManager = (DataManager)data;
        
        // RECURSIVELY ADD TAGS TO STRING
        
        TreeItem root = dataManager.getHTMLRoot();
        fileContents += writeHTMLFile(root);
        
        // WRITE fileContents TO filePath (index.html)
        ...
        try {
            ...
            dialog.show("Error Writing to File", "There was an error writing your data to the temp directory.");
        } finally {
            try{
                if (w != null){
                    w.close();
    ...
    /**
     * This method takes the HTML tag tree within the application and
     * converts it to a single String to be written to a .html file.
     * That file (determined in the above exportData() method) can then
     * be executed by any program which can run HTML files.
     *
    */
    private String writeHTMLFile(TreeItem node){
	
	ObservableList<TreeItem> children = node.getChildren();
        
        String childTags = "";
        for (TreeItem child : children){
            childTags += writeHTMLFile(child);
        }

        return openTag(node) + childTags + closeTag(node);
    }
    
    // HELPER METHOD FOR writeHTMLFile; GENERATES OPENING TAG
    private String openTag(TreeItem node){
        HTMLTagPrototype tag = (HTMLTagPrototype)node.getValue();
        String type = node.getValue().toString();
        type = type.substring(1, type.length()-1);
        String attributes = " ";
        HashMap<String, String> attributesSet = tag.getAttributes();
	Set<String> keys = attributesSet.keySet();
        for (String attributeName : keys) {
            String attributeValue = attributesSet.get(attributeName);
            if(type.contains("text")){
                //text doesn't have any tags
                attributes = " " + attributeValue + " ";
            }else if(type.contains("img") && attributeName.equals("src")){
                //special filepath for image sources
                attributes += " " + attributeName + "=\"" + "images/" + attributeValue + "\" ";
            }else{
                //otherwise just add attributes appropriately
    ...
    // HELPER METHOD FOR writeHTMLFile; GENERATES CLOSING TAG IF NECESSARY
    private String closeTag(TreeItem node){
        HTMLTagPrototype tag = (HTMLTagPrototype)node.getValue();
