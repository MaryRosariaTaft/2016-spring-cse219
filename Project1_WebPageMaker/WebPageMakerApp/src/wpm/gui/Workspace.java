...
public class Workspace extends AppWorkspaceComponent {
	...
	dataManager.reset();
        ...
        Image rmvEl = new Image("file:./images/RemoveElement.png");
        Button tagButton1 = new Button();
        tagButton1.setGraphic(new ImageView(rmvEl));
        tagButtons.add(tagButton1);
        tagButton1.setMaxWidth(BUTTON_TAG_WIDTH);
        tagButton1.setMinWidth(BUTTON_TAG_WIDTH);
        tagButton1.setPrefWidth(BUTTON_TAG_WIDTH);
        tagToolbar.getChildren().add(tagButton1);
        // INIT ITS EVENT HANDLER (REMOVE ELEMENT)
        tagButton1.setOnAction(e -> {
            try{
                pageEditController.handleRemoveElementRequest();
            }catch(IOException ioe){
                // exception should be caught and dialog should 
                // be invoked in handleRemoveElementRequest()
            }
        });
	...
	for (HTMLTagPrototype tag : dataManager.getTags()) {
	    ...
	    // INIT ITS EVENT HANDLER (ADD ELEMENT)
		...
		HTMLTagPrototype clickedTag = dataManager.getTag(tagName);
                try{
                    ...
                    // exception should be caught and dialog should 
                    // be invoked in handleAddElementRequest()
                }
	    });
	}
	...
	cssEditor.textProperty().addListener(e -> {
            try{
                ...
                // exception should be caught and dialog should 
                // be invoked in handleCSSEditing()
            }
	});
	...
	try {
	    ...
	    if (selectedItem != null) {
                //SEE WHICH NODE IS CURRENTLY SELECTED
		...
		Collection<String> keys = attributes.keySet();
                ...
                // DISABLE BUTTONS WHICH SHOULDN'T BE TOUCHED
                Button b = tagButtons.get(0);
                
                // FIRST ENABLE/DISABLE THE REMOVE ELEMENT BUTTON
                b.setDisable(selectedTag.toString().contains("html") ||
                        selectedTag.toString().contains("head") ||
                        selectedTag.toString().contains("title")||
                        selectedTag.toString().contains("link") ||
                        selectedTag.toString().contains("body") );
                
                // THEN ENABLE/DISABLE THE TAG BUTTONS BASED ON WHETHER
                //  THEY'RE LEGAL CHILDREN OF THE SELECTED NODE
                for(int i = 1; i < tagButtons.size(); i++){
                    b = tagButtons.get(i);
                    // newTag REPRESENTS THE BUTTON'S TAG
                    HTMLTagPrototype newTag = dataManager.getTag(b.getText());
                    TreeItem newNode = new TreeItem(newTag);
                    // parentName REPRESENTS THE SELECTED NODE'S TAG
                    String parentName = selectedItem.getValue().toString();
                    parentName = parentName.substring(1,parentName.length()-1);
                    // DISABLE BUTTON IF ITS TAG COULD NEVER BE ADDED
                    //  AS A CHILD OF THE CURRENTLY SELECTED NODE
                    b.setDisable(!newTag.isLegalParent(parentName));
                }
		...
		for (String attributeName : keys) {
			...
			// UPDATE THE TEMP SITE AS WE TYPE ATTRIBUTE VALUES
                        try{
                            ...
                            // exception should be caught and dialog should
                            // be invoked in handleAttributeUpdate()
                        }
                    });
		    row++;
		}
	    }else{
                // DISABLE BUTTONS IF NO ELEMENT IS SELECTED
                for (Button b : tagButtons){
                    b.setDisable(true);
                }
            }
